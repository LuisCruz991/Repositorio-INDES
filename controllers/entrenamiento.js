  ...
  if (JSON.status) {
    ...
    JSON.dataset.forEach(row => {
      ...
      TBODY_ROWS.innerHTML += `
            ...
            <td >
              <button data-modal-toggle = "save-modal" class="rounded-md w-24 h-8 bg-btnactualizar-color font-medium text-btnactualizar-texto dark:text-blue-500 hover:underline" onclick="openUpdate(${row.identrenamiento})">
                Actualizar
              </button>
            <td class="px-6 py-4">
              <button
                class=" rounded-md w-24 h-8 bg-red-500 font-medium text-white dark:text-blue-500 hover:underline" onclick="openDelete(${row.identrenamiento})">
                Eliminar</a>
              </button>
            </td>
          </tr>
...
async function openUpdate(id) {
  ...
  const JSON = await dataFetch(ENTRENAMIENTO_API, 'readOne', FORM);
  // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
  if (JSON.status) {
    ...
    document.getElementById('id').value = JSON.dataset.identrenamiento;
    document.getElementById('fecha').value = JSON.dataset.fecha_entreno;
...
async function openDelete(id) {
  ...
  if (RESPONSE) {
    ...
    const JSON = await dataFetch(ENTRENAMIENTO_API, 'delete', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (JSON.status) {
      // Se carga nuevamente la tabla para visualizar los cambios.
      fillTable();
