...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    const JSON = await dataFetch(MODALIDAD_API, action, FORM);
...
async function openUpdate(id) {
    ...
    if (JSON.status) {
        ...
        document.getElementById('id').value = JSON.dataset.idmodalidad_deporte;
        document.getElementById('nombre').value = JSON.dataset.nombre_modalidad;
    } else {
        sweetAlert(2, JSON.exception, false);
    }
}
...
async function openDelete(id) {
    ...
    const RESPONSE = await confirmAction('¿Desea descartar esta modalidad deportiva?');
    ...
    if (RESPONSE) {
        ...
        const JSON = await dataFetch(MODALIDAD_API, 'delete', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (JSON.status) {
            // Se carga nuevamente la tabla para visualizar los cambios.
            fillTable();
